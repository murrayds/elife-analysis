...
# Two solutions:
# The first is that we do the matching as normal, and then simply lookup the information later
# The second is that we load everything into the tbale at once, and have a different variable name for each

# I will try to get the first to work. 
#
# Load the country infomration .csv file, which was manually created and available along with this file
institution_mapping <- read_csv("https://www.dropbox.com/s/5pcbzhb3v9prwhu/elife_institutions.csv?dl=1")[, 2:3]
country_data <- read_csv("https://www.dropbox.com/s/o2t6724ugp8dzev/country_data_coords.csv?dl=1")
...
elife <- elife %>%
  ...
  mutate(
    ...
    # Mean geographic distance between country of last author and each of the countries of associated reviewers
    ...
    # Mean geographic distance between country of first author and each of the countries of associated reviewers
...
write.csv(elife, "~/Dropbox/eLife/formatted_data/formatted_elife.csv")

write.csv(people, "~/Dropbox/eLife/formatted_data/elife_people.csv")
...
```{r}
elife <- read.csv("~/Dropbox/eLife/formatted_data/formatted_elife.csv")
```
...
ggsave("figures/main/fig_2.tiff", plot = fig2, height = 4, width = 7.5)
ggsave("figures/main/fig_2.png", plot = fig2, height = 4, width = 7.5)
...
ggsave("figures/main/fig_3.png", plot = fig3, height = 5, width = 7)
```
...
ggsave("figures/main/fig_4.png", plot = fig4, height = 5, width = 7)
```
...
data_fig5 <- elife %>%
  ...
  mutate(
    ...
    composition = factor(composition, labels = c("All Men" = "All-male", "All Women" = "All-female", "Mixed" = "Mixed-gender", "Uncertain" = "Uncertain"))
  ) %>%
  filter(composition != "Uncertain")
...
fig5 <- data_fig5 %>% 
  ...
  geom_text(aes(x = 1.5, y = 0.89, label = sig), color = "black") +
...
ggsave("figures/main/fig_5.png", plot = fig5, height = 5, width = 7)
```
...
ggsave("figures/main/fig_6.tiff", plot = fig6, height = 6, width = 7.5)
ggsave("figures/main/fig_6.png", plot = fig6, height = 6, width = 7.5)
```


```{r}
g <- grid.arrange(fig4, fig6, ncol = 2, widths = 1:2)

ggsave("~/Desktop/combined_figure.png", g,  width = 12, height = 6)
```
...
elife %>%
  filter(full_decision_made & has_full_submission & ca_country %in% countries & !is.na(ca_bre_country_homophily)) %>%
  group_by(ca_country, ca_bre_country_homophily) %>%
  summarize(
    paper_count = n(),
    accept_count = sum(accepted == TRUE, na.rm = T),
    encourage_accept_rate = accept_count / paper_count,
  ...
  mutate(ca_country = factor(sapply(as.character(ca_country), simpleCap))) %>%
  # Start ggplot
  ggplot(aes(x = ca_bre_country_homophily, y = encourage_accept_rate, fill = ca_bre_country_homophily)) +
  geom_bar(stat = "identity", position = position_dodge(), color = "black", alpha = 0.8) +
  geom_errorbar(aes(ymin = lower, ymax = upper), color = "black", width = 0.2) +
  geom_text(aes(label = paper_count, y = 0.075), size = 2.5) +
  geom_text(aes(label = sig, x = 1.5, y = 0.95)) +
...
fig7.B <- d1.summ %>%
  ...
  mutate(type = factor(type, 
                       ...
                       labels = c("All Countries", "Excluding U.S.", "Excluding U.S., U.K., and Germany")
                       )
         ) %>%
  ggplot(aes(x = ca_country_homophily, y = encourage_accept_rate, fill = ca_country_homophily)) +
  geom_bar(stat = "identity", position = position_dodge(), color = "black", alpha = 0.8) +
  geom_errorbar(aes(ymin = lower, ymax = upper), color = "black", width = 0.2) +
  geom_text(aes(label = paper_count, y = 0.32), size = 2.5) +
...
```{r}
g <- grid.arrange(fig5, fig7.A, ncol = 2, widths = 1:2)

ggsave("~/Desktop/combined_figure2.png", g,  width = 12, height = 6)
```
...
ggsave("figures/supp/fig_SI1.png", plot = fig_SI.1, height = 4, width = 7)
```
...
fig_SI.2 <- elife %>%
  ...
  geom_text(aes(x = gender, y = measurement + 0.1, label = ifelse(sig %in% c("*", "**"), round(measurement, 3) * 100, "")), color = "black", size = 2.5) +
...
ggsave("figures/supp/fig_SI2.png", plot = fig_SI.2, height = 4, width = 7)
```
  ...
  mutate(
    ...
    composition = factor(composition,
                         labels = c("All men" = "All-male reviewers", "All women" = "All-female reviewers", 
                                    "Mixed" = "Mixed-gender reviewers", "Uncertain" = "Uncertain composition"))
  ) %>%
  # Start ggplot
  ggplot(aes(x = gender, y = acceptance_rate, fill = gender)) +
  geom_bar(stat = "identity", alpha = 0.8, color = "black") +
...
ggsave("figures/supp/fig_SI3.png", plot = fig_SI.3, height = 4, width = 7)
```
...
ggsave("figures/supp/fig_SI4.tiff", plot = fig_SI.4, height = 5, width = 7.5)
ggsave("figures/supp/fig_SI4.png", plot = fig_SI.4, height = 5, width = 7.5)
```
...
ggsave("figures/supp/fig_SI5.png", plot = fig_SI.5, height = 4, width = 7)
```
...
fig_SI.6
ggsave("figures/supp/fig_SI6.tiff", plot = fig_SI.6, height = 4, width = 7.5)
ggsave("figures/supp/fig_SI6.png", plot = fig_SI.6, height = 4, width = 7.5)
```
...
ggsave("figures/supp/fig_SI7.png", plot = fig_SI.7, height = 4, width = 7)
```
