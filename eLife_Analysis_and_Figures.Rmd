---
title: 'Gender and international diversity improves equity in peer review: R Notebook'
...
This notebook documents the analysis of the dataset on peer review outcomes by eLife. Due to the sensitive nature of the data, it is not all made public. However, a sample dataset with all identifying information such as name and instution has been supplied. If you are using this data, run the first, and the third code blocks, skipping the second. 
...
### Gender distirbution of distinct gatekeepers involved in processing full submissions at eLife, includes senior editors, reviewing editors, and invited peer reviewers as %
```{r}
...
ggsave("figures/main/fig_2.png", plot = fig2, height = 4, width = 7)
...
And the same for the content-level tests
```{r}
...
data_fig4 <- elife %>%
  ...
  ungroup() %>%
  gather(measure_key, value = measurement, submit_rate, encourage_accept_rate) %>%
...
fig4 <- data_fig4 %>%
  ...
  geom_text(aes(x = 1.5, y = 0.92, label = ifelse(measure_key == "Submissions", "", as.character(sig))), color = "black") +
  geom_segment(aes(x = 1, xend = 2, y = ifelse(sig != "N/A", 0.85, 1.1), yend = ifelse(sig != "N/A", 0.85, 1.1)), color = "black") +
...
data_fig5 <- elife %>%
  ...
  mutate(
    ...
    composition = factor(composition, labels = c("All Men" = "All male reviewers", "All Women" = "All women reviewers", "Mixed" = "Mixed-gender reviewers", "Uncertain" = "Uncertain"))
  ) %>%
...
fig5 <- data_fig5 %>% 
  ...
  theme_dakota() +
  guides(fill = F) +
  labs(y = "% accepted of full submissions",
...
ggsave("figures/main/fig_6.png", plot = fig6, height = 7, width = 8)
```
...
ggsave("figures/main/fig_7.png", plot = fig7, height = 4, width = 7)
```
...
fig_SI.6
ggsave("figures/supp/fig_SI6.png", plot = fig_SI.6, height = 4, width = 7)
```
