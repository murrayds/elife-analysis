...
gender_homophily <- data_homophily %>%
  ...
  geom_text_repel(aes(hjust = -0.5, vjust = -1, label = paste0(round(acceptance_rate, 1), "%"))) +
  theme_dakota() +
...
countries_separate <- elife %>%
  ...
  geom_text(aes(label = sig, x = 1.5, y = 86), size = 4) +
  geom_segment(aes(x = 1, xend = 2, y = ifelse(sig != "N/A", 80, 101), yend = ifelse(sig != "N/A", 80, 101)), color = "black") +
...
countries_pooled <- d1.summ %>%
  ...
  geom_text(aes(label = sig, x = 1.5, y = 85), size = 4) +
  geom_segment(aes(x = 1, xend = 2, y = ifelse(sig != "N/A", 80, 101), yend = ifelse(sig != "N/A", 80, 101)), color = "black") +
...
full <- elife %>% 
  ...
  summarize(
    submissions = n(),
    prop_homogeny = sum(la_country_homophily, na.rm = T) / submissions
  ) %>%
...
g <- grid.arrange(gender_homophily, g1, ncol = 2)
```
...
ggsave(name, plot = g, height = 8, width = 10)
```
