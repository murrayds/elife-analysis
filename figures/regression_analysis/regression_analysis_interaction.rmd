...
global_tidy <- tidy(full_model, conf.int = T, exponentiate = T) %>%
  mutate(type = "Global Interactions")
...
both.full.plots <- all_models %>%
  filter(type == "Global Interactions" | type == "Main Effects") %>%
  filter(term != "(Intercept)") %>%
  mutate(
    type = factor(type, 
                  levels = c("Main Effects", "Global Interactions"),
                  labels = c("A. Main Effects", "B. Global Interactions")),
    ast = sig2ast(p.value),
...
stargazer(main_effects, standard_interaction, all_male, mixed, full_model,
          ...
          t.auto = F,
          p.auto = F,
          column.labels = c("Main Effects", "With interaction", "Only All-Male", "Only Mixed", "Global Interaction"),
...
ggsave(name, plot = both.full.plots, height = 8, width = 10)
```